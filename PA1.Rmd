...
## changing directory and loading packages

setwd("C:/Users/Vinny/Documents/R/RR")
...
library(caret)
library(stringr)
## in order to avoid scientific notation that sometimes comes up
options(scipen=999)
```
...
2. The mean total number of steps taken per day is `r a` and the median is `r b`.
...
that contains the maximum number of steps is `r c`.
...
The total number of rows containing missing values is `r cc`.
...
The mean total number of steps taken per day is `r e` and the median is `r f`.

In the way that missing values were imputed, with the daily average for the given period there was no change to the rounded mean before `r a` and after `r e`
and a small change to the median `r b` and `r f`.
